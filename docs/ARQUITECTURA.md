...
| `cuentas/` | `cuentas.hooks.js`, `cuentas.servicio.js`, `cuentasCorrientes.rutas.js`, `cuentasCorrientes.pdf.js` | Cuenta del chofer sincronizada, validación de proveedores, saldos y resumen PDF |
| `viajes/` | `viajes.hooks.js`, `viajes.calculos.js` | Liquidación al chofer (FINALIZADO) y débito al pagador (FACTURADO) |
...
| `afterCreate / afterUpdate / afterDelete` | Después de escribir | Los errores se registran en el log, no cortan la respuesta |
...
| `migraciones/` | Cambios incrementales 001…022 | Actualizar producción sin perder datos |
